...
def _check_credits(db, user_id: str, amount: int, action_label: str) -> int:
    ...
    q = int(row["monthly_proposal_quota"] or 0) if row else 0
    if q < amount:
        raise HTTPException(
            status_code=402,
            detail={
                "error": f"{action_label} 크레딧 부족",
                "code": "QUOTA_EXCEEDED",
                "quota_remaining": q,
                "required": amount,
            },
...
def init_db() -> None:
    with get_db() as db:
        db.executescript("""
            ...
            -- ───────── 오늘의 무료 크레딧 (퀴즈 / 운세 / 로또) ─────────
            -- ⚠ Spec 4-B (5/16) 폐기: 시드 + endpoints + 헬퍼 모두 폐기, 테이블만 데이터 보존용 잔존.
...
# ── 크레딧 풀 시드 + 헬퍼 (_seed_credit_pools / _credit_quiz_salt /
#    _normalize_credit_answer / _hash_credit_answer) 은 Spec 4-B (5/16) 폐기.
#    DB 테이블 (credit_quiz_pool / credit_fortune_pool / credit_lotto_daily /
#    credit_attempts) 은 데이터 보존을 위해 유지 (CREATE TABLE 멱등).
...
다음 작업은 사용자가 어떤 표현으로 요청하든 무조건 채팅에서 직접 수행 금지. 안내만 한다.
...
- **입찰 히스토리** ("수주 기록", "패주 기록", "이전 입찰", "히스토리" 등) → "발주처 상세 페이지의 대화 히스토리에서 보세요. 결과를 같이 살펴보시죠."

★ 사용자 의도가 위 6 카테고리 중 하나면, 어떤 표현이든 즉시 안내 모드.
★ 이미 채팅 흐름에서 다른 답변 하고 있어도 사용자가 위 요청하면 안내로 회귀.
★ 크레딧·소요 시간은 절대 언급 금지 (Spec D-Fix-Credit-Msg): 크레딧 잔액·차감량·생성 소요 시간은 화면 UI (사이드바·버튼·알림)가 정확히 안내함. AI는 추정값을 말하지 말 것 (부정확한 자율 생성 방지).

[채팅 내용 — 제안서·산출내역서 영역 자동 반영]
사용자가 채팅에서 합의한 내용 (콘셉트 / 슬로건 / 페이지 분배 / 안전관리 강조 / 차별화 포인트 / 인력 영역 / 예산 의도 등) 은 다음 영역에 자동 반영됨:

✅ ✨ 제안서 생성 — outline 영역 1순위 반영
✅ 산출내역서 생성 — 예산 / 인력 / 기간 영역 1순위 반영

우선순위: 1순위 사용자 대화 > 2순위 RFP 요구사항 > 3순위 AI 자율 판단.

★ 페이지 수 안내 기준 (Spec D-Fix-PageBaseConsistent):
페이지 분배·분량을 안내할 때는 **RFP 에 명시된 페이지 한도(page_limit)** 를 기준으로 한다.
RFP 에 명시가 없으면 **50장** 을 기준으로 안내한다.
임의로 다른 기준(예: "보통 30페이지", "통상 80페이지" 같은 자율 추정)을 만들지 마라.
이 기준은 ✨ 제안서 생성 모달의 권장 분량 배지(RFP 매수 또는 50) 와 정합되어야 한다.
예: RFP 한도 50 → "50장 기준으로 공연 N / 홍보 N / ... 어떻게 나눌까요?"
    RFP 한도 명시 X → "기본 50장 기준으로 어떻게 나눠볼까요?"

사용자가 본 영역 질문 시 정직 답변:
- "✨ 누르면 우리 대화 내용이 담겨?" → "네, outline 영역 1순위로 반영돼요."
- "산출내역서도 우리 대화 반영돼?" → "네, 예산·인력·기간 의도 그대로 반영돼요."
- "지금 너랑 이야기하는 의미 있어?" → "있어요. 우리 대화가 outline 근간으로 들어가요. 단 본문 작성은 ✨ 버튼이 합니다 — 저는 채팅에서 제안서 본문을 쓰지 않아요."

금지 답변 (시스템 실 동작이랑 정합 X):
- ❌ "반영 안 돼요"
- ❌ "RFP 원문만 보고 생성"
- ❌ "채팅 내용은 학습 안 함"
- ❌ "복붙해야 함" / "별도로 전달해야 함"

★ 단, "제안서 만들자 / 써보자 / 쓰자 / 작성해줘 / 초안 뽑아줘" 같이 채팅에서 직접 본문 작성을 요청하는 어조는 위 [별도 기능과의 분리] 룰이 절대 우선한다. "대화가 outline 근간"이라는 이유로 채팅에 제안서 본문(콘셉트·슬로건·페이지별 내용)을 풀어쓰지 마라. 자동 반영은 사용자가 ✨ 제안서 생성 버튼을 누른 후에 일어난다. 채팅에서 할 일은 "✨ 제안서 생성 버튼을 눌러주세요" 안내까지다.

[RFP 인식 — 정직 우선]
- 받은 정보: RFP 원문 전체 + 분석 요약본 둘 다 학습됨.
- "RFP 학습했어?" / "RFP 다 봤어?" 질문 시 정직히: "네, RFP 원문 전체 학습했어요."
- 세부 조항 ("3.2.1항", "특수 조건", "담당자 이름") 질문 시 원문에서 직접 인용·답변.
- 모호 회피 금지. "분석 요약만 받았다" / "주요 부분만 받았다" 같은 거짓 답변 금지.
- RFP 에 명시되지 않은 정보는 "RFP 에 명시 없어요" 로 솔직 답변. 거짓 정보 생성 금지.
...
orientation 엄격 규칙 (Spec D-Fix-Orient):
- 제안서/사업계획서/문서 "형식·규격·방향"에 대한 명시만 판단 근거.
- ★ 맥락 구분: 다른 대상(행사 포스터·현수막·배너·전시물·간판 등)의 세로/가로 규격은 무시. 제안서/사업계획서/문서 형식에 대한 방향만 반영 (역방향 사고 방지).
- 제안서 형식 명시 패턴:
  · "가로 / landscape / A4 가로 / 가로형 / 가로 방향" → "landscape"
  · "세로 / portrait / A4 세로 / A4 긴 면 / 긴 면 / 세로형 / 세로 방향" → "portrait"
...
[category — 제출 단계 분류]
각 서류를 아래 5개 중 하나로 분류하세요. 입찰 진행 단계 기준입니다.
- "입찰참가": 입찰에 참가하기 위한 행정 서류 (입찰참가신청서, 참가신청 확인서, 참여 확약서, 위임장, 사용인감계 등)
- "자격증빙": 참가 자격을 증명하기 위해 발급기관에서 떼어 제출하는 서류 (사업자등록증, 법인인감증명서, 법인등기부등본, 경쟁입찰참가등록증, 중소기업확인서, 신용평가등급확인서, 납세증명서, 직접생산확인증명서 등)
- "제안서": 제안서 평가와 직접 관련된 서류 (제안서 표지, 제안업체 일반현황, 전문인력보유현황, PT 발표자료 표지, 이동식 저장장치(USB), 그리고 이들의 증빙서류인 재직증명서·건강보험자격득실확인서·학력증명서·자격증 사본 등)
- "계약시": 낙찰 후 계약 체결 시점에 제출하는 서류 (근로자 권리보호 서약서, 성희롱·성폭력 예방 서약서, 안전보건 서약서 등 — RFP에 "계약 시 제출" 등으로 명시된 것)
- "기타": 위 분류에 명확히 들어가지 않는 서류
판단이 애매하면, 그 서류가 입찰 과정의 어느 시점에 필요한지를 기준으로 가장 가까운 분류를 선택하세요.
...
def _startup() -> None:
    ...
    # ── 크레딧 풀 시드 (_seed_credit_pools 호출) 은 Spec 4-B (5/16) 폐기.
...
# ── /api/credit/* — 오늘의 무료 크레딧 (퀴즈 / 운세 / 로또) 은 Spec 4-B (5/16) 폐기.
#    DB 테이블 / 인덱스 / users.credit_count 컬럼 은 데이터 보존을 위해 유지 (CREATE TABLE 멱등).

# KST 타임존 (UTC+9) — SMS 영역 + 다른 영역 KST 시각 정합 영역 사용
_KST = timezone(timedelta(hours=9))


def _today_kst_str() -> str:
    """KST 기준 오늘 날짜 'YYYY-MM-DD' — quota 리셋 / SMS 영역 등 공용 헬퍼."""
...
# 기존 패턴 활용:
#   환경변수 영역 SALT + HMAC-SHA256 — 자립 영역 (Spec 4-B credit 영역 폐기로 의존성 0)
...
⚠⚠⚠ 제안서 ↔ 산출내역서 정합 (입찰 critical) ⚠⚠⚠

[★★★ 우선순위 (최종, 본 directive 가 컨텍스트 영역 옛 우선순위 문구보다 우선) ★★★]
1순위 — 사용자 ↔ AI 대화 (예산/인력/기간 명시 시 그대로 override)
2순위 — 제안서 outline 안 명시된 인력 / 장비 / 시설 / 일정 (제안서 기반 정합 강제)
3순위 — RFP 예산 한계 + 요구사항 (한계 영역, 콘텐츠 기준 X)

[제안서 outline 강제 반영 — outline 안 명시 = 그대로 반영. 자율 판단으로 새 항목 만들지 X]
- 인력 수: 제안서 "운영 인력 8명" → 산출내역서도 8명 (10명·12명 X). 직책 구성도 동일 (PM/AD/디자이너 구성 그대로).
- 장비/시설 사양: 제안서 "메인 무대 6m × 10m" → 동일 사양 (4m × 8m 변경 X). LED·음향 채널 수도 동일.
- 강조 카테고리/콘텐츠: 제안서 "체험존 6개소" / "야간 미디어아트" → 산출내역서 영역 필수 카테고리 (누락 X).
- 일정/기간: 제안서 "3일간 운영" → period_qty=3, period_unit="일" 그대로.

⚠ 두 문서 (제안서 ↔ 산출내역서) 정합 X = 평가위원 교차 검증 영역 신뢰도 ↓ = 입찰 critical 사고.
⚠ outline 영역 명시 X 항목만 = 업계 시세 기반 자율 산정 (다만 합리 범위 + 한도 영역).
...
def api_admin_users_list(
    ...
    with get_db() as db:
        # credit_count 은 Spec 4-B (5/16) 폐기로 응답에서 제외 (DB 컬럼은 보존).
        rows = db.execute(
