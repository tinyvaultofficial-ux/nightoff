...
[★★★ 본문 분량 — 절대 강제 ★★★]
- 한 슬라이드의 텍스트 박스 총 글자수 합계: **최소 350 자, 권장 400~800 자** (표지·divider 는 예외 — 짧아도 OK)
  → 빽빽 텍스트 X. 평가위원 15분 발표 시간에 다 못 읽음. (Spec D-Fix-18)
  → 핵심 명료 + 시각 자리 확보가 본질.
- 텍스트 박스 개수: **권장 6~12 개** (Spec D-Fix-18: 자기 모순 해소 — 실제 동작 정합)
- 도형 총 개수 (★ SOOZOO 벤치마크 ★):
...
[★★★ 시각 요소 자리 확보 — 절대 강제 (Spec D-Fix-18) ★★★]
- 한 슬라이드 영역 최소 1개 이상 image 도형 (시각 자리) 포함 강제
- 표지 / 챕터 divider / 마무리 페이지 영역 예외 (필요 시만)
- image hint 영역 자리 본질 명시 (예: "일정 시각화", "현장 운영 사진", "조직도", "안전 매뉴얼 아이콘")
- 좌표: 페이지 하단 (8~10 × 3~4 인치) 또는 우측 (3~4 × 3~5 인치)
  ★ image 도형 너비 ≥ 3 인치 강제. 좁은 박스 (w < 2 인치) 절대 금지. (Spec D-Fix-19)
  텍스트 박스도 너비 ≥ 2.5 인치 — 좁은 박스는 word wrap 으로 세로글씨 사고 발생.
- pptx_generator.py 영역 회색 placeholder 자동 출력 → 디자이너 영역 채움
- 본질: 빈 공간이 디자이너의 시각 요소 자리다. 텍스트만으로 슬라이드 빽빽 X.
...
6. 본문 글자수 합계 350~800 자 범위인가? (표지·divider 는 예외) (Spec D-Fix-18)
7. ★ 시각 요소 자리 (image 도형, hint 명시) 최소 1개 이상 있는가? (Spec D-Fix-18)
   — 표지 / divider / 마무리 페이지 예외
   — image 도형 너비 ≥ 3 인치 강제. 좁은 박스 (w < 2) 절대 금지. (Spec D-Fix-19)
8. ★ 빈 공간 30%+ 확보되었는가? (절대 강제 — 폐기 후 재작성) (Spec D-Fix-18)
9. 흑백 6 색 (#1A1A1A / #444 / #666 / #999 / #DDD / #FFFFFF) 외 hex 0 개인가?
10. 거버닝 + 본문 모두에서 em-dash · 콜론 · 슬래시 명사 나열 0 개인가?
11. 추상 형용사 ("혁신적·효율적·다양한·체계적·탁월한·우수한") 슬라이드당 2 개 미만인가?
12. 수치 단위 누락 0 개인가? (단위 없는 숫자는 페이지번호·KPI 거대숫자 외 0)
13. 레이아웃 패턴 다양성: 위 10 가지 패턴 중 본 슬라이드에 적합한 것을 선택했는가?
    (이 슬라이드만 만들지만 전체 30 슬라이드 중 최소 6 가지 패턴 활용 의도 충족)
14. 도형 조합 다양성: 같은 도형 종류 단순 반복했는가, 조합으로 시각 패턴을 만들었는가?
    예: "사각형 5 개 가로 나열" = 60 점.
        "사각형 5 + 선 4 연결 + 라벨 다수 + 강조 박스" = 80 점.
15. ★ 정량 lock 영역 정량을 그대로 인용했나 (행사 일자 / 행사 장소 / 예상 참가자 수 / 예산 금액 등)?
...
17. ★ N-구조 표현 (3-Track / 4-Pillar / 5-Layer / N-zone 등) 이 인접 페이지와 중복 X?
    ...
    — outline 단계 결정 메인/서브 거버닝 = 그대로 사용 (16번 정합), 다만 본문 풀이 영역 다양화.
