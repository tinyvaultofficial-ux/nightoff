...
```json
{
  ...
  "outline": [
    ...
    },
    {
      ...
      "viz_hint": "stat (KPI 3~4개) + comparison(AS-IS/TO-BE) — 본문 간략 (1~3 문장 권장) + 박스 + 통계. 제안서 초반이므로 장황한 설명 회피"
    },
    {
      "page": 5,
      "section": "Ⅰ.3 제안의 특장점",
      "governing": "추상 메시지 (예: '관객과의 양방향 소통')",
      "key_msgs": ["접근 방향 1 (추상)", "접근 방향 2 (추상)", "접근 방향 3 (추상)"],
      "viz_hint": "추상 메시지 + 접근 방향 3~4개 박스 (정성). ⚠ 정량 수치·자사 실적·회사명 절대 금지 (마스터 원칙 영역 2). 도메인별 차별화 패턴 예시: festival 체험중심='관객과의 양방향 소통' / conference 학술='지식 교류 깊이' / forum 외교='글로벌 연대' / exhibition='콘텐츠 깊이' / music='공연 품질'. AI 자율 판단."
    },
    ...
  ]
}
...
★ 메인 거버닝 (정성적 — 페이지의 의미·가치·통합 메시지)
- 25 자 이내, 명사형 문어체
- 페이지 본질을 한 문장으로 — "이 페이지가 무슨 가치를 전달하는가"
- 예: "안전을 우선하는 운영 체계", "현장과 디지털을 잇는 참여 구조"
- ⚠ 메인은 수치 X — 정성 통합 메시지만

★ 서브 거버닝 (정량적 — 구체 수치·사양·통계)
- 짧게, 단위까지 명시
- 메인 거버닝의 근거 — "왜 그 메시지가 가능한가"
- 예: "조명 8000W × 12 채널", "설문 응답률 91% (n=1,247)"
- ⚠ 서브는 추상 X — 정량만

⚠ 역할 뒤섞이면 안 됨 — 메인이 수치로, 서브가 추상으로 가는 것 모두 금지.
