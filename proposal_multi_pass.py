...
RFP 가 목차 없이 자유 형식이면 표준 흐름 (장수는 가변, RFP/도메인 따라 조정):
  ...
  4. ★ 컨셉 (1~2장 권장 — 슬로건 페이지 + 구체화 페이지, RFP 컨셉 분량 따라 가변)
...
11) 컨셉 슬로건 페이지 — 거대 슬로건 + 부제 1~2줄, 도형 5~15. 적용: 컨셉 페이지 첫 장.
    슬로건 폰트 size 동적 매핑 (슬라이드 가로 11.69 인치 초과 방지):
      · 5단어 이내 (한글 짧은 슬로건): 80~90pt 권장
      · 6~8단어 (영문 또는 한글 중간): 60~70pt 권장
      · 9단어 이상 (긴 슬로건): 50pt 또는 줄바꿈 (2줄 분할) 권장
    영문 슬로건은 한글보다 가로 길이 더 큼 — 보수적 size 선택.
