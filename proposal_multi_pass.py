...
- 텍스트 박스 개수: **최소 8 개, 권장 15~30 개**
- 도형 총 개수: **15~50 개** (표·카드·플로우·KPI 같이 풍부)
...
허용 색상은 다음 6 개뿐:
  ...
  · #999 (메타·푸터)     · #DDD (구분선) · #FFFFFF (배경·반전 텍스트)
...
[캔버스 — 16:9 강제]
- slide_width=13.33, slide_height=7.5 (16:9) — user prompt 에 주어진 값 그대로 사용
- 모든 도형 좌표는 캔버스 안 (0 ≤ x+w ≤ slide_width, 0 ≤ y+h ≤ slide_height)
- 다른 비율은 사용자가 명시적으로 요청한 경우만

[푸터 일관성 — 모든 슬라이드 공통]
- y=7.4 (또는 slide_height 의 ~99%) 에 가로 구분선 #DDD
- 좌하단: 회사명 + section 명 (9pt #999, weight 300~400)
- 우하단: "현재페이지 / 전체페이지" (9pt #999, weight 300~400)

[레이아웃 패턴 가이드 — 참고용]
표지: 좌 검은 바 (0.4×전체높이) + 큰 헤드라인 36~60pt + 부제 + 우측 KPI 2~3 개
목차: 번호 (Ⅰ~Ⅴ) 60pt + 항목명 24pt + 한 줄 요약 12pt — 5 행
챕터 divider: 거대 번호 (Ⅰ) 180~220pt 가운데 + 챕터명 32pt + 한 줄 14pt
KPI 페이지: 큰 숫자 (80~120pt, weight 900) × 3~4 개 카드, 라벨·부연 설명 함께
표 페이지: rect 로 셀 + text 채우기. 행 6~10, 열 4~6
플로우: 박스/원 + 잇는 line + 끝 arrow. 단계 5~7 개
비교 (AS-IS/TO-BE): 좌우 박스 + 가운데 화살표, 각 박스 5~7 행
조직도: 최상위 박스 + 선 + 하위 박스 3~5 개, 각 박스 = 직책 + 경력
...
[도형 검증 자가 점검 — 슬라이드 작성 후 반드시 확인 — 10 항목]
...
4. 푸터 (가로선 + 회사명/섹션 + 페이지번호) 빠진 슬라이드 없는가?
5. 한 슬라이드 도형 수 5~30 (표지·divider 는 5~15) 범위인가?
