...
[5부 구조 — 도메인 무관 고정]
   Ⅰ. 제안 개요     (제안 배경 / 과업 범위 / 제안의 특징·장점 / 컨셉 히어로 + 컨셉 풀이)
   ...
   + 디자이너 영역 placeholder (자동 삽입 — outline 단계에서 명시):
       · 컨셉 풀이 직후 [2D 키비주얼 시안 자리] 1~2장
       · 무대·공간 계획 직후 [3D 공간조성 연출컷 자리] 2~3장

[표준 제안서 흐름 — RFP 우선, 없으면 표준]
★ 절대 원칙: RFP 가 목차 / 평가 항목 명시 → 그대로 따름. AI 임의 변경 X.

RFP 가 목차 없이 자유 형식이면 표준 흐름:
  1. 표지 (1장)
  2. 목차 (1장)
  3. 사업 이해 / 추진 배경 / 과업 범위 (2~4장)
  4. ★ 컨셉 히어로 (1장 — 거대 슬로건만)
  5. ★ 컨셉 풀이 (1장 — 4분면/3-Track/Pillar 구체화)
  6. ★ [2D 키비주얼 시안 자리] (1~2장 — 디자이너 영역 placeholder)
  7. 구체 제안 — 프로그램 / 운영 / 안전 / 실적 (10~20장)
  8. ★ [3D 공간조성 연출컷 자리] (2~3장 — 무대·공간 페이지 직후)
  9. 마무리 (1장 — 감사합니다)

[도메인별 흐름 정합 — festival/forum/exhibition/conference 검증됨]
★ festival (행사·축제):
  사업이해 → 컨셉 히어로 → 컨셉 풀이 → [2D 시안] →
  프로그램 구성 → 무대·공간 → [3D 시안] →
  운영 → 안전 → 홍보 → 실적 → 마무리
...
표지에 포함하는 것:
- 과업명 (Paperlogy 9Black, 60~80pt) — 핵심
- 부제 (사업 부제 또는 발주처명, Paperlogy 5Medium, 18~24pt)
- 날짜 (Paperlogy 4Regular, 12~14pt)

★ 표지에 포함하지 않는 것 (디자이너 영역 + 청렴제):
- 회사명 / 제안사명 / 로고 placeholder
- 회사 슬로건 / 회사 비전
- 주소 / 연락처

도형 수: 15~30 개 (표지·챕터·마무리 범위), 빈 공간 풍부.

[마무리 슬라이드 가이드 — 마지막 1장, 단순 구조]
마무리에 포함하는 것:
- "감사합니다" (Paperlogy 9Black, 100~120pt) — 거대 텍스트
- 부제 (Paperlogy 5Medium, 20~28pt) — "[발주처명]을 위해 최선을 다하겠습니다" 또는 도메인 어미 변형

★ 마무리에 포함하지 않는 것 (청렴제 + 디자이너 영역):
- 회사 정보 박스 (회사명·주소·전화·이메일)
- 자사 PM / 담당자 이름
- 가짜 실적 ("○○건 진행", "○○회 수행" 등)
- 회사 로고 / 로고 placeholder

도형 수: 15~30 개, 빈 공간 풍부.
...
11) 컨셉 히어로 — 거대 슬로건 (Black 80pt+) + 부제 1~2줄, 도형 5~15. 적용: 컨셉 페이지 첫 장 (Ⅰ장 마지막)
12) 컨셉 풀이 — 4분면 / 3-Track / Pillar 구체화 + 행사 운영 연결, 도형 30~70. 적용: 컨셉 히어로 직후
