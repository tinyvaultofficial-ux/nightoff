...
1) AS-IS / TO-BE 비교 — 좌우 2 단 + 가운데 화살표. 적용: 문제→해결, 변화 전후
   ★ 프리셋 옵션 (Spec D-Fix-Preset3 / Spec Governing-Title-Three-Presets): 본 패턴은 slide_data 출력 시 preset='two_column' + title(선택, 강력 권장 — 페이지 상단 거버닝 한 줄, 25~40자, 자동 브랜드 보라) + columns=[{title, items[]}, {title, items[]}] (정확히 2개 / 좌=AS-IS / 우=TO-BE) 형식 사용 가능 — 코드가 좌우 패널 + 중앙 화살표를 안정 배치 (옵트인 / preset 미지정 시 기존 자율 그대로). ★ 최상위 title 은 페이지 거버닝, columns[i].title 은 패널 라벨(AS-IS/TO-BE 등) — 다른 필드. 예: {"preset": "two_column", "title": "기존 운영을 참여 중심으로 전환한다", "columns": [{"title": "AS-IS", "items": ["일방 전달", "단기 이벤트"]}, {"title": "TO-BE", "items": ["양방향 참여", "지속 거버넌스"]}]}
...
3) 단계별 프로세스 (가로 흐름) — 단계 5-7 개, 박스 + 화살표. 적용: 추진 절차, 운영 흐름
   ★ 프리셋 옵션 (Spec D-Fix-Preset2 / Spec Governing-Title-Three-Presets): 본 패턴은 slide_data 출력 시 preset='process' + title(선택, 강력 권장 — 페이지 상단 거버닝 한 줄, 25~40자, 자동 브랜드 보라) + steps=[{label, desc(선택)}] (3~7개) 형식 사용 가능 — 코드가 chevron 을 가로 균등 정렬 (옵트인 / preset 미지정 시 기존 자율 그대로). 예: {"preset": "process", "title": "5단계 표준 프로세스로 안정적 실행", "steps": [{"label": "분석", "desc": "현황 진단"}, {"label": "기획", "desc": "전략 수립"}, {"label": "실행", "desc": "단계별 추진"}]}
...
9) 정량 데이터 강조 — 큰 숫자 (Black 80pt+) + 라벨. 적용: 통계, 성과, KPI
   ★ 프리셋 옵션 (Spec D-Fix-Preset1 / Spec Governing-Title-Three-Presets): 본 패턴은 slide_data 출력 시 preset='quantitative' + title(선택, 강력 권장 — 페이지 상단 거버닝 한 줄, 25~40자, 자동 브랜드 보라) + metrics=[{value, label}] (1~3개) 형식 사용 가능 — 코드가 큰 숫자 좌표를 안정 배치 (옵트인 / preset 미지정 시 기존 자율 그대로). 예: {"preset": "quantitative", "title": "사업 성과를 세 개 정량으로 요약", "metrics": [{"value": "50억", "label": "총 예산"}, {"value": "5만 명", "label": "예상 참여"}]}
