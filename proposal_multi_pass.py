...
[skeleton_id 배정 규칙 — Spec D-Build-SkeletonConnect]
운영이 사람이 양질 제안서에서 떠낸 검증된 골격 13종 HTML 을 R2 에서 동기화해뒀다.
...
분배 규칙:
① 페이지 내용(governing_main / key_msgs) 과 골격 use 가 자연스럽게 일치하면 그 골격 우선.
② 같은 skeleton_id 를 연속 2장 배정 금지 (다양성 — 직전 페이지와 다른 ID).
