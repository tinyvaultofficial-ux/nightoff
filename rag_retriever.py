...
def format_hints_for_prompt(hints: dict) -> str:
    ...
    Spec D-Fix-RAGNumberBlock (수치 환각 차단):
      이전 모드: "디테일·수치·구조 그대로 흉내" — RAG 발췌의 정량 수치까지 LLM 이 차용
        (예: 발췌의 "촬영 1,200만원" → 새 제안서의 "출연료 1,200만원" 슬롯 차용 환각).
      현재 모드: "형식·워딩·논리만 차용, 수치 인용 금지" — 수치 자리는 반드시
        [RFP 분석] 블록 또는 [정량 lock] 블록의 수치만 사용.
      이로써 형식 신호(표 패턴·카드 구성·문장 구조) 는 유지하되 수치 누출만 막는다.
    """
    if not hints:
        return ""
    lines = [
        ...
        "발주처·사업·금액·수치는 모두 **다른 사업의 것** 이다. **문장 구조, 표·카드 구조, 어휘 패턴** 만",
        "이 회사가 실제로 수주에 성공한 작법이다. 새 제안서는 **같은 구조·표현 결** 로 쓰되,",
        "**수치는 절대 차용하지 마라** (수치는 아래 [수치 인용 절대 금지] 영역 참조).",
        "",
        "★★★ 강제 준수 사항 ★★★",
        "1. 새 제안서의 슬라이드 본문은 아래 발췌와 **같은 정도로 빽빽** 해야 한다.",
        "   추상 형용사(혁신적·효율적·다양한)로 채우지 말고 구체적 실명·단계·방법으로 채워라.",
        "2. 아래 발췌의 **문장 구조·표 패턴·카드 구성·논리 전개·어휘 결** 을 그대로 흉내내라.",
        "   (예: '4단계 매뉴얼: 평시→주의→경계→위험' / 'D-90→D+30' 같은 구조 패턴)",
        "3. **★★★ [발췌 수치 인용 절대 금지] (Spec D-Fix-RAGNumberBlock) ★★★**",
        "   발췌에 등장하는 모든 정량 수치는 새 제안서에 인용 금지. 대상 (포함하나 이에 한정 X):",
        "     · 금액 (○○만원 / ○○억원 / ○○원)",
        "     · 인원 / 팀 수 (○○명 / ○○팀 / ○○개사 / ○○대)",
        "     · 관람객 / 관객 / 참여자 수 (○○명 / ○○만 명)",
        "     · 비율·% (송출률 ○○% / 전환율 ○○% / 만족도 ○○/5.0)",
        "     · 출연료·인건비·운영비 등 (출연료 약 ○○ / 인력 ○○명 투입)",
        "     · 도달·노출·CTR 같은 마케팅 KPI",
        "     · 전년 실적 수치 (전년 ○○명 / 누적 ○○회)",
        "   → 발췌의 수치를 슬롯만 바꿔 가져오지 마라 (예: 발췌의 '촬영 1,200만원' 을",
        "      새 제안서의 '출연료 1,200만원' 으로 차용 금지).",
        "   → 수치 자리는 반드시 **[RFP 분석] 블록 또는 [정량 lock] 블록의 수치만** 사용. 둘 다 없는",
        "      자리는 수치 비우고 \"어떻게 할지\" 구체적 방법·메커니즘으로 채워라 (D-Fix-NoHallucinatedKPI 정합).",
        "4. 단위 표기 자체는 유지 — 수치는 단위까지(㎡, 명, 원, %, 시간, 회 등). 단 그 수치는 RFP/lock 출처.",
        "5. 발주처명·회사명·사람 이름·고유명사도 베끼지 마라. **구조와 표현 결만** 새 제안서에 적용.",
        "6. 한 슬라이드 본문이 너무 짧으면(추상적·뼈만) **즉시 폐기 후 재작성** — 발췌 수준의 분량으로 확장하되 수치는 RFP/lock 만.",
        "",
    ...
    if hints["sample_excerpts"]:
        ...
        lines.append("")
        lines.append("─────────────────────────────────────────────────────")
        lines.append("위 발췌의 **단계 구분 / 표 행 수 / 카드 개수 / 문장 길이·결 / 논리 전개** 그대로 흉내.")
        lines.append("단 **수치는 위 발췌에서 인용 금지** — 수치 자리는 [RFP 분석] 블록 또는 [정량 lock] 블록에서만 인용.")
        lines.append("**본문 분량과 구체성** 은 발췌 수준으로 채우되, 구체성은 **수치 대신 \"어떻게 할지\" 방법·메커니즘** 으로.")
        lines.append("─────────────────────────────────────────────────────")
    lines.append("")
